...
export const AnalysisResultPage = () => {
  ...
  const tabItems = task?.result
    ? [
        { key: 'breakdown', label: '页面拆解', children: <PageBreakdown result={task.result} /> },
        { key: 'diff', label: '差异分析', children: <DiffAnalysis result={task.result} /> },
        { key: 'fusion', label: '融合建议', children: <FusionSuggestions result={task.result} /> },
        { key: 'changes', label: '变化报告', children: <ChangeReport taskId={task.id} /> },
        { key: 'figma', label: 'Figma 结构', children: <FigmaStructure result={task.result} /> },
      ]
    : [];
