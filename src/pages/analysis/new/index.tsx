...
const MOCK_OWN_KEYFRAMES: KeyframeItem[] = [
  { time: '00:00', label: '起始页面', description: '我方页面录屏起始画面', changeType: 'start' },
  { time: '00:04', label: '页面切换', description: '强变化，页面切换或弹窗出现', changeType: 'strong' },
  { time: '00:09', label: '局部变化', description: '中等变化，局部内容或状态切换', changeType: 'medium' },
  { time: '00:16', label: '页面切换', description: '强变化，页面切换或弹窗出现', changeType: 'strong' },
  { time: '00:20', label: '结束页面', description: '我方页面录屏结束画面', changeType: 'end' },
];
        ...
        <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Space size={4}>
            ...
            {'competitor' in recognition ? (
              ...
              <Typography.Text style={{ fontSize: 12, color: '#389E0D' }}>
                识别为：<strong>{recognition.pageType}</strong> · {(recognition as OwnPageRecognitionResult).description.slice(0, 20)}...
              </Typography.Text>
            )}
          </Space>
...
export const NewAnalysisPage = () => {
  ...
  const [competitorKeyframesCollapsed, setCompetitorKeyframesCollapsed] = useState(false);
  ...
  const [ownHasVideo, setOwnHasVideo] = useState(false);
  const [ownKeyframesCollapsed, setOwnKeyframesCollapsed] = useState(false);
                  ...
                  {ownHasVideo && ownRecognizingUids.size === 0 && (
                    <div style={{ marginTop: 16 }}>
                      <KeyframeStrip
                        ...
                        onToggle={() => setOwnKeyframesCollapsed((v) => !v)}
                        accentColor="#1677FF"
                        label="我方页面关键帧"
                      />
                    </div>
          ...
          <div style={{ width: 288, flexShrink: 0, position: 'sticky', top: 24 }}>
            <Card style={{ borderRadius: 8, border: '1px solid #F0F0F0' }} bodyStyle={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <Typography.Title level={5} style={{ margin: 0 }}>业务场景</Typography.Title>
              </div>
              {currentRecognition ? (
                ...
                {mixedFiles.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '10px 14px',
                        background: item.recognizing ? '#FAFAFA' : item.category === 'competitor' ? '#FFFBF5' : '#F0F7FF',
                        borderRadius: 8,
                        border: `1px solid ${item.recognizing ? '#F0F0F0' : item.category === 'competitor' ? '#FFD591' : '#BAE0FF'}`,
                        transition: 'all 0.3s',
                      }}
                    >
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <Space size={8}>
                          {item.recognizing ? (
                            <Space size={4}>
                              <Spin indicator={<LoadingOutlined style={{ fontSize: 12, color: '#1677FF' }} />} />
                              <Typography.Text style={{ fontSize: 12, color: '#1677FF' }}>识别中...</Typography.Text>
                            </Space>
                          ) : (
                            <>
                      ...
                      {!item.recognizing && item.category === 'competitor' && item.competitorRecognition && (
                        <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
                          ...
                          <Typography.Text style={{ fontSize: 11, color: '#389E0D' }}>
                            识别为：<strong>{item.competitorRecognition.competitor}</strong> · {item.competitorRecognition.flowOrPage} · 置信度 {item.competitorRecognition.confidence}%
                          </Typography.Text>
                        </div>
                      )}
                      {!item.recognizing && item.category === 'own' && item.ownRecognition && (
                        <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
                          <CheckCircleOutlined style={{ color: '#52C41A', fontSize: 11 }} />
                          <Typography.Text style={{ fontSize: 11, color: '#389E0D' }}>
                            识别为：<strong>我方 {item.ownRecognition.pageType}</strong> · 置信度 {item.ownRecognition.confidence}%
                          </Typography.Text>
                        </div>
                      )}
                    </div>
